...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <br /><br /><hr />
      <a id="wd-update-todo-completed" href={`${API}/${todo.id}/completed/${todo.completed}`}
        className="btn btn-primary float-end">
        Complete Todo ID = {todo.id}</a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
      <input type="checkbox" checked={todo.completed} className="float-start mt-2"
        onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} />
      <br /><br /><hr />
      <a id="wd-update-todo-description" href={`${API}/${todo.id}/description/${todo.description}`}
        className="btn btn-primary float-end">
